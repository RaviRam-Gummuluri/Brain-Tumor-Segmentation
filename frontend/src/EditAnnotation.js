...
function EditAnnotation({  }) { 
    ...
    const [brainImage, setBrainImage] = useState(state?.brainImage || null);
    const [annotationImage, setAnnotationImage] = useState(state?.annotationImage || null);
    const [originalAnnotationImage, setOriginalAnnotationImage] = useState(state?.annotationImage || null);
    ...
    const handleReset = () => {
        setAnnotationImage(originalAnnotationImage); 
        const canvas = canvasRef.current;
        const ctx = canvas.getContext('2d');
        drawImageOnCanvas(originalAnnotationImage); 
        setHistory([]); 
    };
    ...
    const processAnnotationImage = (annotationImgBase64) => {
        const img = new Image();
        img.src = annotationImgBase64; // Directly pass the image as base64
        img.onload = () => {
            const canvas = canvasRef.current;
            const ctx = canvas.getContext('2d');
            ...
            // Process image data to convert all non-black regions to red
            for (let i = 0; i < imageData.data.length; i += 4) {
                const [r, g, b, a] = [
                    imageData.data[i],
                    imageData.data[i + 1],
                ...
                // If pixel is not black (r, g, b all equal to 0)
                if (!(r === 0 && g === 0 && b === 0)) {
